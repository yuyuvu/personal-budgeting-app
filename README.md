...
.\gradlew.bat build
```
Для быстрого ручного тестирования можно также использовать уже добавленного пользователя. 
! Для этого перед запуском приложения скачайте папку personal_budgeting_appdata с её содержимым из данного репозитория и поместите 
скачанную personal_budgeting_appdata в той же папке, где лежит собранный / скачанный jar.
В кошелёк пользователя newuser1 уже добавлены некоторые операции и бюджеты. Пароль данного пользователя такой же, как и логин - newuser1.
