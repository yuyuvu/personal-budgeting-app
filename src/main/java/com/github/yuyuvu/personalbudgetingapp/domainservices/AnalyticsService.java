...
import static com.github.yuyuvu.personalbudgetingapp.presentation.ColorPrinter.resetColor;
...
public class AnalyticsService {
    ...
    if (balance < 0) {
      ...
      result
          .append(
              paintGreen(
                  String.format(
                      "Баланс доходов и расходов: %s%.2f", resetColor(), wallet.getBalance())))
          .append("\n");
    }
    return result.toString();
  }
  ...
  public static String makeExpensesSummary(Wallet wallet) {
    ...
    result
        .append(
            paintGreen(
                String.format("Общие расходы: %s%.2f", resetColor(), wallet.getTotalExpenses())))
        .append("\n");
          ...
          .append(
              paintYellow(
                  String.format(
                      ...
                      WalletOperationsService.getExpensesByCategory(wallet, category))))
          .append("\n");
    }
    return result.toString();
  }
  ...
  public static String makeIncomeSummary(Wallet wallet) {
    ...
    result
        .append(
            paintGreen(
                String.format("Общие доходы: %s%.2f", resetColor(), wallet.getTotalIncome())))
        .append("\n");
          ...
          .append(
              paintYellow(
                  String.format(
                      "\t- %s: %.2f",
                      capitalizeFirstLetter(category),
                      WalletOperationsService.getIncomeByCategory(wallet, category))))
          .append("\n");
    }
    return result.toString();
  }
    ...
    for (Map.Entry<String, Double> category : wallet.getBudgetCategoriesAndLimits().entrySet()) {
      result.append(
          paintPurple(
              String.format(
                  "\t- %s: %.2f. ",
                  capitalizeFirstLetter(category.getKey()),
                  BudgetingService.getLimitByCategory(wallet, category.getKey()))));
  ...
  public static String makeIncomeWalletOperationsList(Wallet wallet) {
    ...
    if (wallet.getIncomeWalletOperations().isEmpty()) {
      result
          .append(
              paintYellow(
                  " - Нет добавленных операций дохода, "
                      + "добавьте новые операции в меню управления доходами и расходами."))
          .append("\n");
      return result.toString();
    }
    ...
    for (Wallet.WalletOperation wo : wallet.getIncomeWalletOperations()) {
      result
          .append(
              paintYellow(
                  String.format(
                      "- ID: %d, Категория: \"%s\", Сумма: %.2f, Дата: %s.",
                      wo.getId(),
  ...
  public static String makeExpensesWalletOperationsList(Wallet wallet) {
    ...
    if (wallet.getExpensesWalletOperations().isEmpty()) {
      result
          .append(
              paintYellow(
                  " - Нет добавленных операций расхода, "
                      + "добавьте новые операции в меню управления доходами и расходами."))
          .append("\n");
      return result.toString();
    }
  ...
  public static String makeSummaryByCategories(
    ...
    if (!incorrectCategories.isEmpty()) {
      result.append(
          paintPurple(
              String.format(
                  "Обратите внимание, были запрошены категории %s, "
    ...
    result
        .append(
            paintGreen(
                String.format(
                    "Суммарные %s по категориям \"%s\": %s%.2f",
    ...
    result
        .append(
            paintCyan(
                String.format(
                    "%s по каждой из существующих запрошенных категорий:",
                    (isIncome ? "Доходы" : "Расходы"))))
        .append("\n");
    ...
    if (WalletOperationsService.getWalletOperationsByCategories(
      ...
      result
          .append(
              paintYellow(
                  "\tНет учтённых операций по запрошенным категориям, "
                      + "добавьте новые операции в меню управления доходами и расходами."))
          .append("\n");
      return result.toString();
    }
    ...
    if (isIncome) {
      ...
      result
          .append(
              paintGreen(
                  String.format(
                      "Суммарный бюджет по запрошенным категориям: %s%.2f",
                      resetColor(),
                      BudgetingService.getLimitByCategories(
                          wallet, false, requestedCategories.toArray(new String[0])))))
          .append("\n");
      result.append(paintGreen("Оставшийся суммарный бюджет по запрошенным категориям: "));
      double remainderTotal =
          BudgetingService.getRemainderByCategories(
              wallet, false, requestedCategories.toArray(new String[0]));
        ...
        if (wallet.getBudgetCategoriesAndLimits().containsKey(category)) {
          result.append(
              paintPurple(
                  String.format(
                      "\t- %s: %.2f. ",
                      capitalizeFirstLetter(category),
                      BudgetingService.getLimitByCategory(wallet, category))));
          result.append(paintPurple("Оставшийся бюджет: "));
          ...
          result
              .append(
                  paintPurple(
                      String.format(
                          "\t- %s: %s. ", capitalizeFirstLetter(category), "бюджет не задан")))
              .append("\n");
        }
      }
    }
  ...
  public static String makeSummaryByPeriod(
    ...
    StringBuilder result = new StringBuilder();
    result.append("\n");
    if (WalletOperationsService.getWalletOperationsByPeriod(wallet, periodStart, periodEnd)
        .isEmpty()) {
      result
          .append(paintYellow("Нет добавленных операций дохода или расхода за указанный период."))
          .append("\n");
      result
          .append(paintYellow("Добавьте новые операции в меню управления доходами и расходами."))
          .append("\n");
      return result.toString();
    }

    // Сводка за период по доходам
    ArrayList<Wallet.WalletOperation> incomeOperationsDuringPeriod =
        WalletOperationsService.getWalletOperationsByTypeAndPeriod(
            wallet, true, periodStart, periodEnd);
    result
        .append(
            paintGreen(
                String.format(
    ...
    if (incomeOperationsDuringPeriod.isEmpty()) {
      result
          .append(paintYellow("\tНет учтённых операций дохода за указанный период."))
          .append("\n");
    } else {
      for (String category : wallet.getWalletOperationsIncomeCategories()) {
        ArrayList<Wallet.WalletOperation> foundOperations =
            WalletOperationsService.getWalletOperationsByTypeAndPeriodAndCategory(
                wallet, true, category, periodStart, periodEnd);
    ...
    ArrayList<Wallet.WalletOperation> expensesOperationsDuringPeriod =
        WalletOperationsService.getWalletOperationsByTypeAndPeriod(
            wallet, false, periodStart, periodEnd);
    result
        .append(
            paintGreen(
                String.format(
    ...
    if (expensesOperationsDuringPeriod.isEmpty()) {
      result
          .append(paintYellow("\tНет учтённых операций расхода за указанный период."))
          .append("\n");
    } else {
      for (String category : wallet.getWalletOperationsExpensesCategories()) {
        ArrayList<Wallet.WalletOperation> foundOperations =
            WalletOperationsService.getWalletOperationsByTypeAndPeriodAndCategory(
                wallet, false, category, periodStart, periodEnd);
        if (!foundOperations.isEmpty()) {
          result
              .append(
                  paintYellow(
    ...
    result
        .append(
            paintCyan(
                "Список всех учтённых операций дохода или расхода за указанный период "
                    + "(отсортированы по типу и дате):"))
        .append("\n");
    for (Wallet.WalletOperation wo :
        WalletOperationsService.getWalletOperationsByPeriod(wallet, periodStart, periodEnd).stream()
            .sorted(Comparator.comparing(Wallet.WalletOperation::getDateTime))
            .sorted(Comparator.comparing(Wallet.WalletOperation::isIncome).reversed())
            .toList()) {
      result
          .append(
              paintYellow(
                  String.format(
                      "- ID: %d, Тип: %s, Категория: \"%s\", Сумма: %.2f, Дата: %s.",
                      wo.getId(),
