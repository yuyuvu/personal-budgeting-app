          ...
          if (reportFormat.isEmpty()) {
            ...
            pathToReportFile =
                DataPersistenceService.saveAnalyticsReportToFile(
                    ...
                    makeFilenameForReportFile("summary_by_categories"),
                    ".txt");
        ...
        case "2" -> {
          ...
          if (reportFormat.isEmpty()) {
            ...
            pathToReportFile =
                DataPersistenceService.saveAnalyticsReportToFile(
                    deleteColorsFromString(displayedUserNameForReports + reportContent),
                    makeFilenameForReportFile("summary_by_period"),
                    ".txt");
            printlnGreen(String.format("Отчёт успешно сохранён в \"%s\"!", pathToReportFile));
          }
        }
                ...
                DataPersistenceService.saveAnalyticsReportToFile(
                    deleteColorsFromString(
                        displayedUserNameForReports
                            + AnalyticsService.makeIncomeWalletOperationsList(wallet)),
                    makeFilenameForReportFile("income_operations_list"),
                    ".txt");
            printlnGreen(String.format("Отчёт успешно сохранён в \"%s\"!", pathToReportFile));
          }
        }
          ...
          if (reportFormat.isEmpty()) {
            ...
            pathToReportFile =
                DataPersistenceService.saveAnalyticsReportToFile(
                    deleteColorsFromString(
                        displayedUserNameForReports
                            + AnalyticsService.makeExpensesWalletOperationsList(wallet)),
                    makeFilenameForReportFile("expenses_operations_list"),
                    ".txt");
            printlnGreen(String.format("Отчёт успешно сохранён в \"%s\"!", pathToReportFile));
          }
        }
        case "5" -> PersonalBudgetingApp.setCurrentMenu(new AnalyticsMenu());
  ...
  private String handleRequestByCategories(Wallet wallet) throws CancellationRequestedException {
    ...
    do {
      try {
        printCyan(
            String.format(
                "Введите названия категорий %s, по которым требуется вывод. Указывайте их в кавычках и через пробел: ",
        ...
        if (getCurrentUserInput()
          ...
          requestedCategories =
              Arrays.stream(requestedCategories)
                  .map(s -> s.replaceAll("\"", ""))
                  .map(String::strip)
                  .distinct()
                  .toArray(String[]::new);
          printlnCyan(
              "Введены категории: "
                  + Arrays.toString(requestedCategories).replaceAll("[\\[\\]]", "")
                  + ".");
        } else {
