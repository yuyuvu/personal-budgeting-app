 ...
 * функционал. Также позволяет выйти из аккаунта. Выводит уведомления. Позволяет скрыть уведомления
 * во всём приложении или вернуть их обратно.
 */
public class AppMainMenu extends Menu {
  ...
  public void showMenu() {
    ...
    println(
        "5. Скрыть / вернуть уведомления (сейчас: "
            + paintPurple(
                ConfigManager.checkNotificationsConfigForCurrentUser()
                        .equals(ConfigManager.BooleanPropertiesValues.TRUE.name())
                    ? "отображаются"
                    : "скрыты")
            + ").");
    println("6. Выход из аккаунта.");
  ...
  public void handleUserInput() {
    ...
    try {
      ...
      switch (getCurrentUserInput()) {
        ...
        case "5" -> {
          if (ConfigManager.reverseNotificationsConfigForCurrentUser()) {
            printlnPurple("Теперь уведомления снова будут отображаться во всём приложении.");
          } else {
            printlnPurple("Теперь уведомления скрыты во всём приложении.");
          }
        }
        case "6" -> logOutOfCurrentUser(true);
